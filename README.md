...
#### AI-Powered Face Recognition · Anomaly Detection · Web-Based Administration
...
The system was designed, developed, and validated as a Final Year Project at
...
- Automatic event logging with duplicate suppression
...
- Student-facing portal for self-service access
...
├── Rule 2: Confidence < 45% → MEDIUM severity
...
| Multi-Image Enrollment | 6–7 images per student for robust coverage |
...
| Phase 3 | iPhone (USB) | 6–7 | Standard | 65–75% |
| Phase 3 Optimized | iPhone (USB) | 6–7 | Good | **80%+** |

### Anomaly Detection Performance

| Rule | Detection Rate | False Flag Rate |
|------|---------------|-----------------|
| Unknown Person Detection | **100%** | 0% |
| Late-Night Entry Detection | **100%** | 0% |
| Low-Confidence Detection | **90%** | 10% |
...
> **Key Finding:** The two most impactful improvements were switching from a laptop
> webcam to an iPhone camera (+5–10% accuracy) and increasing enrollment images
> from 2–3 to 6–7 per student (+15–20% accuracy).
...
│   ├── app.py                      # Main Flask app + API routes + CameraRecognizer
...
Laptop webcam gave < 50% accuracy	System unusable for security	Switched to iPhone via USB → 80%+ accuracy
...
Adjabi et al. (2020) — ResNet encoders outperform classical methods
